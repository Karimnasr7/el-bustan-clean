...
export function LoginForm() {
  // تم حذف: const [confirmPassword, setConfirmPassword] = useState('');
  // تم حذف: const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        // التعديل هنا: تخزين التوكن الحقيقي المشفر بدلاً من كلمة 'true'
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-black">
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-300">يرجى إدخال كلمة المرور للوصول</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* تم حذف حقل تأكيد كلمة المرور بالكامل من هنا */}
